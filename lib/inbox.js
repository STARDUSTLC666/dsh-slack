...
/** 创建收件箱队列。 */
export function createInboxQueue(capacity = INBOX_CAPACITY) {
...
export class InboxQueue {
    ...
    seen = new Set();
    capacity;
    constructor(capacity = INBOX_CAPACITY) {
 ...
 */
export function parseMessageEvent(raw, botUserId) {
...
function unwrapMessageEvent(raw) {
    if (!isObject(raw))
        return null;
    const type = raw.type;
    if (type === 'message')
    ...
    if (isObject(inner) && inner.type === 'message')
        return inner;
    return null;
}
